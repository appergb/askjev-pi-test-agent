...
test('Pi SDK workflow, Mock labeling, findings and unchanged regression work together', { skip: process.platform !== 'darwin', timeout: 30000 }, async () => {
  ...
  try {
    ...
    call = 0;
    const capped = await runTask({ ...task, budget: { ...task.budget, max_model_turns: 5 } }, config, { outputRoot: path.join(root, 'runs'), progress: () => {} });
    assert.equal(capped.statistics.model_turns, 5, JSON.stringify(capped)); assert.equal(call, 5, 'No sixth HTTP request is sent');
    assert.equal(capped.run_status, 'completed', JSON.stringify(capped));
    call = 0;
    const unavailable = { ...config, scoring: { mode: 'real', baseUrl: 'http://127.0.0.1:1' } };
    const degraded = await runTask({ ...task, scoring_failure: 'all' }, unavailable, { outputRoot: path.join(root, 'runs'), progress: () => {} });
    assert.equal(degraded.run_status, 'completed', JSON.stringify(degraded));
    assert.equal(degraded.scoring.backend_mode, 'unavailable');
    assert.equal(degraded.statistics.executed_cases, 1);
    assert.equal(degraded.scoring.fallback.policy, 'all');
    assert.equal(degraded.assessment, 'confirmed_findings');
    call = 0;
    const strict = await runTask(task, unavailable, { outputRoot: path.join(root, 'runs'), progress: () => {} });
    assert.equal(strict.run_status, 'failed'); assert.equal(strict.statistics.executed_cases, 0);
    await fs.writeFile(path.join(project, 'math.mjs'), 'export const add=(a,b)=>a+b;');
