...
当前版本 **1.4.0，工程试用阶段**。支持明确授权的 JavaScript 模块与本地静态前端 Chrome 功能测试。测试在本地执行，云端提供生成与评分服务。

[安装技能](docs/skill-installation.md) · [CLI](docs/cli.md) · [会话管理](docs/sessions.md) · [评分架构设计](docs/architecture/scoring-selection-v2.md) · [NVIDIA 技术清单](docs/architecture/nvidia-stack.md)

## 使用你的 ChatGPT Codex 安装

**请使用你的 ChatGPT Codex 安装该技能。** 将独立安装包 `askjev-agent-skill-1.4.0.tar.gz` 交给具备本地文件和终端能力的编码 Agent：
...
- 检测饱和、并列、缺失上下文和模型分歧，不可靠时停用模型评分特征。
...
部署参数与历史性能观测见 [技术清单](docs/architecture/nvidia-stack.md)。本轮只读核对服务，未改动云端配置。
...
当前支持选定 JavaScript 模块和本地静态前端；模型可见输入 16 KB、快照 512 KB。评分服务 4096-token 上限与生成服务的长上下文能力分开管理。尚不支持生产网站、支付/登录流程、后端 API、视觉差异测试或执行步骤断点恢复。新架构中的 hybrid、评分降级、独立复核、根因自动去重与校准训练仍是待实现能力。
