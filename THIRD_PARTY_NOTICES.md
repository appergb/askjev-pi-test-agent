...
运行时依赖 `@earendil-works/pi-coding-agent`、`@earendil-works/pi-ai` 与终端组件 `@earendil-works/pi-tui`，版本均为 0.85.1。已安装包的元数据声明 MIT 许可，源码位于 [earendil-works/pi](https://github.com/earendil-works/pi)。依赖通过 npm 原样安装，未将其源码重新署名。上游许可与著作权声明应随相应组件保留。
