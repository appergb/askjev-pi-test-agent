...
You are the coding agent; askJEV Agent generates and executes business tests. Use the bundled CLI rather than writing replacement tests yourself. When installation is requested, run `node <this-skill-directory>/scripts/install.mjs`. It verifies and installs the bundled runtime, all approved runtime Skills, the askjev / askJEV / pi-test-agent script aliases, and the askjev-cli interactive terminal. Dependencies come from the locked package; Node.js >=22.19 must already be present. Do not claim that copying SKILL.md itself executed an installer.
